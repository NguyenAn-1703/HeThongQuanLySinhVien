...
public class DiemSinhVienDao
{
    ...
    public List<(int HocKy, string Nam)> GetDanhSachKyHoc(int maSinhVien)
    {
        ...
        while (reader.Read())
        {
            result.Add((reader.GetInt32("HocKy"), reader.GetString("Nam")));
        }
    ...
    public (float DiemTBHe10, float DiemTBHe4, int TongTinChi) TinhDiemTrungBinhTheoKy(int maSinhVien, int hocKy, string nam)
    {
        using var conn = MyConnection.GetConnection();
        ...
        if (reader.Read())
        {
            float diemTBHe10 = reader.IsDBNull(0) ? 0 : reader.GetFloat(0);
            float diemTBHe4 = reader.IsDBNull(1) ? 0 : reader.GetFloat(1);
            int tongTinChi = reader.IsDBNull(2) ? 0 : reader.GetInt32(2);
