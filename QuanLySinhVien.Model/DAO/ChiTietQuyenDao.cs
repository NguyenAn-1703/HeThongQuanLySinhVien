...
public class ChiTietQuyenDao
{
    ...
    {
        if (_instance == null) _instance = new ChiTietQuyenDao();
        return _instance;
    }
    ...
    public bool Insert(ChiTietQuyenDto chiTietQuyenDto)
    {
        ...
        using (var conn = MyConnection.GetConnection())
        {
            var query = @"INSERT INTO chitietquyen (MaCN, MaNQ, HanhDong)
    ...
    public bool HardDelete(int maNQ, int maCN, string hanhDong)
    {
        ...
        using (var conn = MyConnection.GetConnection())
        {
            var query = @"DELETE FROM chitietquyen
                             WHERE MaNQ = @MaNQ
                             AND MaCN = @MaCN
                             AND HanhDong = @HanhDong
                            ;";
    ...
    public ChiTietQuyenDto GetById(int maNQ, int maCN, string hanhDong)
    {
        ...
        using var cmd = new MySqlCommand(
            "SELECT MaCN, MaNQ, HanhDong FROM chitietquyen WHERE Status = 1 AND MaNQ = @MaNQ AND MaCN = @MaCN AND HanhDong = @HanhDong ",
            conn);
