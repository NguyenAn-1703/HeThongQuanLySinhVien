...
public class HocPhiTinChiDao
{
    ...
    {
        if (_instance == null) _instance = new HocPhiTinChiDao();
        return _instance;
    }
    ...
    public List<HocPhiTinChiDto> GetAll()
    {
        ...
        using var cmd =
            new MySqlCommand(
                "SELECT MaHPTC, MaNganh, SoTienMotTinChi, ThoiGianApDung FROM hocphitinchi WHERE Status = 1", conn);
    ...
    public bool Insert(HocPhiTinChiDto hocPhiTinChiDto)
    {
        ...
        using (var conn = MyConnection.GetConnection())
        {
            var query = @"INSERT INTO hocphitinchi (MaNganh, SoTienMotTinChi, ThoiGianApDung)
