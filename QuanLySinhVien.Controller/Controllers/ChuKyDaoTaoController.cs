...
public class ChuKyDaoTaoController
{
    ...
    {
        if (_instance == null) _instance = new ChuKyDaoTaoController();
        return _instance;
    }
    ...
    {
        var year = nienKhoa.Split("-");
        var startYear = Convert.ToInt32(year[0]);
    ...
    public bool Validate(string nienKhoa)
    {
        var year = nienKhoa.Split("-");
        var startYear = Convert.ToInt32(year[0]);
        if (_chuKyDaoTaoDao.GetByStartYear(startYear) == null) return false;
        return true;
    }
    ...
    public ChuKyDaoTaoDto GetByStartYearEndYear(string startendyear)
    {
        ...
        int startYear = short.Parse(years[0]);
        int endYear = short.Parse(years[1]);
