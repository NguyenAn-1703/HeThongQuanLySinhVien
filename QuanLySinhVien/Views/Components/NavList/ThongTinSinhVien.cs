...
public class ThongTinSinhVien:NavBase
{
    ...
    private void InitComponent()
    {
        Console.WriteLine("InitComponent");
         ...
         for (int i = 0; i < 9; i++)
         {
             leftInformation1.RowStyles.Add(new RowStyle(SizeType.Percent, 100f / 9));
         }
         ...
         for (int i = 0; i < 5; i++)
         {
             leftInformation2.RowStyles.Add(new RowStyle(SizeType.Percent, 100f / 5));
         }
         ...
         for (int i = 0; i < 4; i++)
         {
             leftInformation3.RowStyles.Add(new RowStyle(SizeType.Percent, 100f / 4));
         }
         ...
         for (int i = 0; i < 3; i++)
         {
             panelDiemTB.RowStyles.Add(new RowStyle(SizeType.Percent, 33.33F));
         }
         ...
         panelDiemTB.Controls.Add(new Label { Text = "Điểm TB (Hệ 10):", Font = GetFont.GetFont.GetMainFont(10f, FontType.Bold), Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft }, 0, 0);
         panelDiemTB.Controls.Add(lblDiemTBHe10, 1, 0);
         
         panelDiemTB.Controls.Add(new Label { Text = "Điểm TB (Hệ 4):", Font = GetFont.GetFont.GetMainFont(10f, FontType.Bold), Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft }, 0, 1);
         panelDiemTB.Controls.Add(lblDiemTBHe4, 1, 1);
         
         panelDiemTB.Controls.Add(new Label { Text = "Tổng tín chỉ:", Font = GetFont.GetFont.GetMainFont(10f, FontType.Bold), Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft }, 0, 2);
    ...
    private void LoadAvatar(string avatarPath)
    {
        ...
        catch (Exception ex)
        {
            ...
            Bitmap errorImage = new Bitmap(200, 200);
    ...
    private void LoadDanhSachKyHoc(int maSinhVien)
    {
        try
        {
            ...
            {
                foreach (var (hocKy, nam) in danhSachKy)
                {
                    cboKyHoc.Items.Add($"Kỳ {hocKy} - Năm {nam}");
                }
    ...
    private void CboKyHoc_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        string selectedText = cboKyHoc.SelectedItem?.ToString() ?? "";
        if (selectedText == "Chưa có dữ liệu") return;
        
        try
        {
            var parts = selectedText.Split('-');
            int hocKy = int.Parse(parts[0].Replace("Kỳ", "").Trim());
            string nam = parts[1].Replace("Năm", "").Trim();
