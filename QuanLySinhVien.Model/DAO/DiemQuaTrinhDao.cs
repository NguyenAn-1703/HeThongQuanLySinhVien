...
public class DiemQuaTrinhDao
{
    ...
    {
        if (_instance == null) _instance = new DiemQuaTrinhDao();
        return _instance;
    }
    ...
    public bool Insert(DiemQuaTrinhDto diemQuaTrinhDto)
    {
        ...
        using (var conn = MyConnection.GetConnection())
        {
            var query = @"INSERT INTO DiemQuaTrinh (MaKQ, DiemSo)
                             VALUES (@MaKQ, @DiemSo)";
    ...
    public bool HardDelete(int maDQT)
    {
        ...
        using (var conn = MyConnection.GetConnection())
        {
            var query = @"DELETE FROM DiemQuaTrinh
                         WHERE MaDQT = @MaDQT";
            using (var cmd = new MySqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@MaDQT", maDQT);
