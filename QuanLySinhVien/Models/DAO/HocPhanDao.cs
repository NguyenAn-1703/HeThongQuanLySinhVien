...
public class HocPhanDao
{
    ...
    public List<HocPhanDto> GetAll()
    {
        ...
        using var cmd = new MySqlCommand("SELECT MaHP, MaHPTruoc, TenHP, SoTinChi, HeSoHocPhan, SoTietLyThuyet, SoTietThucHanh FROM hocphan WHERE Status = 1", conn);
    ...
    public bool Insert(HocPhanDto hocPhanDto)
    {
        ...
        {
            // status = 1, id auto +1
            string query = @"INSERT INTO hocphan (MaHPTruoc, TenHP, SoTinChi, HeSoHocPhan, SoTietLyThuyet, SoTietThucHanh)
    ...
    public HocPhanDto GetHocPhanById(int maHP)
    {
        ...
        using var conn = MyConnection.GetConnection();
        using var cmd =
            new MySqlCommand("SELECT MaHP, MaHPTruoc, TenHP, SoTinChi, HeSoHocPhan, SoTietLyThuyet, SoTietThucHanh FROM hocphan WHERE Status = 1 AND MaHP = @MaHP",
                conn);
        cmd.Parameters.AddWithValue("@MaHP", maHP);
        using var reader = cmd.ExecuteReader();
    ...
    public static void TestHocPhanDAO()
    {
        try
        {
            ...
            bool insertResult = dao.Insert(newHocPhan);
            ...
            // 5️⃣ Test Delete
            bool deleteResult = dao.Delete(getByIdResult.MaHP);
